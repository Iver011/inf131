...
class Pokemon(pygame.sprite.Sprite):
    ...
    def perform_attack(self, other, move):
        ...
        damage = ((2 * self.level / 5 + 2) * move.power * 
                  (self.attack / other.defense)) / 50 + 2

        
        stab = 1.5 if move.type in self.types else 1
        
        
        effectiveness = calculate_effectiveness(move.type, other.types)

        critical = 2 if random.randint(1, 24) == 1 else 1

        random_factor = random.uniform(0.85, 1.0)

        
        damage = math.floor(damage * stab * effectiveness * critical * random_factor)
...
def calculate_effectiveness(move_type, defender_types):
    ...
    for defender_type in defender_types:
        if defender_type in effectiveness_chart.get(move_type, {}):
            effectiveness *= effectiveness_chart[move_type][defender_type]
...
level = 30
bulbasaur = Pokemon('Bulbasaur', level, 25, 150) #carterpie rattata ekans pikachu nidoran
...
def evaluate_moves(pokemon, opponent, moves=None, index=0, max_damage=0, best_move=None):
    if moves is None:
        moves = pokemon.moves
    ...
    cache_key = (move.name, tuple(pokemon.types), tuple(opponent.types))

    
    if cache_key in damage_cache:
        damage = damage_cache[cache_key]
    else:
        # Calcular el daño si no está en el cache
        base_damage = ((2 * pokemon.level / 5 + 2) * move.power *
                       (pokemon.attack / opponent.defense)) / 50 + 2
        stab = 1.5 if move.type in pokemon.types else 1
        effectiveness = calculate_effectiveness(move.type, opponent.types)

        
        if effectiveness == 0:
            return evaluate_moves(pokemon, opponent, moves, index + 1, max_damage, best_move)
...
rival_last_move = None

def rival_turn():
    global game_status, rival_last_move

    
    all_moves = rival_pokemon.moves
    best_move = evaluate_moves(rival_pokemon, player_pokemon)

    
    valid_moves = [move for move in all_moves if move != rival_last_move]
    ...
    if best_move in valid_moves:
        move_to_use = best_move
    else:
        
        move_to_use = random.choice(valid_moves)

    
    rival_pokemon.perform_attack(player_pokemon, move_to_use)
    rival_last_move = move_to_use  
